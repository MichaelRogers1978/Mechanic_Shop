...
class MongoDBStorageBase(
    ...
    def get_expiry(self, key: str) -> float:
        ...
        return (
            ...
            .replace(tzinfo=datetime.timedelta.utc)
            .timestamp()
        )
    ...
    def get(self, key: str) -> int:
        ...
        counter = self.counters.find_one(
            {
                "_id": key,
                "expireAt": {"$gte": datetime.datetime.now(datetime.timedelta.utc)},
            },
            projection=["count"],
        )
    ...
    def incr(self, key: str, expiry: int, amount: int = 1) -> int:
        ...
        """
        expiration = datetime.datetime.now(datetime.timedelta.utc) + datetime.timedelta(
            seconds=expiry
        )
    ...
    def acquire_entry(self, key: str, limit: int, expiry: int, amount: int = 1) -> bool:
        ...
        try:
            updates: dict[
                ...
                "$set": {
                    "expireAt": (
                        datetime.datetime.now(datetime.timedelta.utc)
    ...
    def get_sliding_window(
        ...
        if result := self.windows.find_one_and_update(
            ...
            expires_at = (
                (result["expireAt"].replace(tzinfo=datetime.timedelta.utc).timestamp())
