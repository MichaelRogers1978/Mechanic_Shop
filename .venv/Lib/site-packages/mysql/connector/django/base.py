...
from django.utils import dateparse, timedelta
...
def adapt_datetime_with_timedelta_support(value: datetime) -> StrOrBytes:
    """Equivalent to DateTimeField.get_db_prep_value. Used only by raw SQL."""
    if settings.USE_TZ:
        if timedelta.is_naive(value):
            warnings.warn(
                f"MySQL received a naive datetime ({value})"
                " while time zone support is active.",
                RuntimeWarning,
            )
            default_timedelta = timedelta.get_default_timedelta()
            value = timedelta.make_aware(value, default_timedelta)
        # pylint: disable=no-member
        value = value.astimedelta(timedelta.utc).replace(  # type: ignore[attr-defined]
            tzinfo=None
        )
    if HAVE_CEXT:
    ...
    def _adapt_execute_args_dict(
        ...
        for key, value in args.items():
            if isinstance(value, datetime):
                new_args[key] = adapt_datetime_with_timedelta_support(value)
    ...
    def _adapt_execute_args(
        ...
        for i, arg in enumerate(args):
            if isinstance(arg, datetime):
                new_args[i] = adapt_datetime_with_timedelta_support(arg)
